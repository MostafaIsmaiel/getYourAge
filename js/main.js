window.onload = function () {
  ...
  function getAge(day, month, year) {
    ...
    let getAgeYear = Math.floor(
        (date - setYear) / 1000 / 60 / 60 / 24 / 365.25
      ), // Get age year
      getAgeMonth = Math.floor(
        (((date - setYear) / 1000 / 60 / 60 / 24 / 365.25) * 12) % 12
      ), // Get age month
      // Get age day
      getAgeDay = () => {
        if (currentDay >= day) {
    ...
    if (
      parseInt(day) > currentMonthDays ||
      enterYear.value == "year" ||
      enterMonth.value == "month" ||
      enterDay.value == "Day" ||
      getAgeDay() < 0 ||
      getAgeMonth < 0
    ) {
      Object.assign(resultSuccess.style, {
        opacity: "0",
        visibility: "hidden",
      });

      Object.assign(resultDanger.style, {
        opacity: "1",
        visibility: "visible",
      });

      resultDanger.innerHTML = `Please enter a valid date...`;
    } else {
      Object.assign(resultDanger.style, {
        opacity: "0",
        visibility: "hidden",
      ...
      resultSuccess.innerHTML = `Your age is ${getAgeYear} Years: ${getAgeMonth} Months : ${getAgeDay()} Days on ${
        week[weekDay]
      }.`;
  ...
  btn.addEventListener("click", () =>
    getAge(enterDay.value, monthResult, enterYear.value)
  );
};
